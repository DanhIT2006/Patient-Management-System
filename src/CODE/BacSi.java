...
public class BacSi extends JFrame {
    ...
    private String selectedMaBenhNhan = null; // Lưu mã bệnh nhân được chọn
    ...
    private void initUI() {
            ...
            accountLabel.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    infoItem.addActionListener(evt -> {
                        cardLayout_tk.show(mainPanel_tk, "account");
                    });
        ...
        JLabel lblTitle_ctbn = new JLabel("Thông tin chi tiết bệnh nhân", SwingConstants.CENTER);
        ...
        // Thêm trường nhập mã bệnh nhân
        JLabel lblSelectMaBN = new JLabel("Mã bệnh nhân:");
        JTextField tfSelectMaBN = new JTextField(13);
        JButton btnLoadPatient = new JButton("Tải thông tin");

        gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 1;
        contentPanel.add(lblSelectMaBN, gbc);
        gbc.gridx = 1; contentPanel.add(tfSelectMaBN, gbc);
        gbc.gridx = 2; contentPanel.add(btnLoadPatient, gbc);
        ...
        gbc.gridheight = 1;
        gbc.gridx = 1; gbc.gridy = 1; contentPanel.add(new JLabel("Mã bệnh nhân:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfMaBN, gbc);

        gbc.gridx = 1; gbc.gridy = 2; contentPanel.add(new JLabel("Họ và Tên:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfHo, gbc);

        gbc.gridx = 1; gbc.gridy = 3; contentPanel.add(new JLabel("Giới tính:"), gbc);
        gbc.gridx = 2; contentPanel.add(cbGioiTinh, gbc);

        gbc.gridx = 1; gbc.gridy = 4; contentPanel.add(new JLabel("Ngày sinh:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfNgaySinh, gbc);

        gbc.gridx = 1; gbc.gridy = 5; contentPanel.add(new JLabel("Số CCCD:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfCMND, gbc);

        gbc.gridx = 1; gbc.gridy = 6; contentPanel.add(new JLabel("SĐT:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfSDT, gbc);

        gbc.gridx = 1; gbc.gridy = 7; contentPanel.add(new JLabel("Nghề nghiệp:"), gbc);
        gbc.gridx = 2; contentPanel.add(tfNgheNghiep, gbc);

        gbc.gridx = 3; gbc.gridy = 1; contentPanel.add(new JLabel("Ngày vào viện:"), gbc);
        gbc.gridx = 4; contentPanel.add(tfNgayKham, gbc);

        gbc.gridx = 3; gbc.gridy = 2; contentPanel.add(new JLabel("Mã bác sĩ:"), gbc);
        gbc.gridx = 4; contentPanel.add(tfMaBS, gbc);

        gbc.gridx = 3; gbc.gridy = 3; contentPanel.add(new JLabel("Bác sĩ phụ trách:"), gbc);
        gbc.gridx = 4; contentPanel.add(tfBacSi, gbc);

        gbc.gridx = 3; gbc.gridy = 4; contentPanel.add(new JLabel("Yêu cầu khám:"), gbc);
        gbc.gridx = 4; contentPanel.add(new JScrollPane(taYeuCauKham), gbc);

        gbc.gridx = 3; gbc.gridy = 5; contentPanel.add(new JLabel("Đối tượng:"), gbc);
        gbc.gridx = 4; contentPanel.add(cbDoiTuong, gbc);

        gbc.gridx = 3; gbc.gridy = 6; contentPanel.add(new JLabel("Địa chỉ:"), gbc);
        ...
        btnLoadPatient.addActionListener(e -> {
            selectedMaBenhNhan = tfSelectMaBN.getText().trim();
            if (selectedMaBenhNhan.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Vui lòng nhập mã bệnh nhân!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        Runnable loadAppointments = () -> {
            if (selectedMaBenhNhan == null || selectedMaBenhNhan.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn mã bệnh nhân trước!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }

            tableModel.setRowCount(0);
            try (Connection conn = ConnectionDatabase.getConnection()) {
                System.out.println("Querying appointments for ma_benh_nhan: " + selectedMaBenhNhan);

                String query1 = "SELECT * FROM benhnhan WHERE mabenhnhan = ?";
                PreparedStatement stmt1 = conn.prepareStatement(query1);
                stmt1.setString(1, selectedMaBenhNhan);
                ...
                if (rs1.next()) {
                    ...
                    infoPanel.add(new JLabel("Mã bệnh nhân: " + selectedMaBenhNhan));
                ...
                stmt2.setString(1, selectedMaBenhNhan);
        ...
        // Xử lý nút tải về
        btnTaiVe.addActionListener(e -> generateAndDownloadWordDocument());
        ...
        recordMenu.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (!isLoggedIn || UserSession.userId == null || !UserSession.role.equals("bacsi")) {
                    ...
                    dispose();
                    new Login().setVisible(true);
                } else if (selectedMaBenhNhan == null || selectedMaBenhNhan.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Vui lòng chọn mã bệnh nhân trước!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                } else {
    ...
    private void loadPatientData(JTextField tfMaBN, JTextField tfHo, JComboBox<String> cbGioiTinh,
        ...
        try (Connection conn = ConnectionDatabase.getConnection()) {
            ...
            stmtBenhNhan.setString(1, selectedMaBenhNhan);
            ...
            stmtPhieuKham.setString(1, selectedMaBenhNhan);
    ...
    private void loadMedicalRecordData() {
        ...
        try (Connection conn = ConnectionDatabase.getConnection()) {
            ...
            stmtHoso.setString(1, selectedMaBenhNhan);
    ...
    private void generateAndDownloadWordDocument() {
        ...
        try {
            ...
            String filePath = "HosoBenhAn_" + selectedMaBenhNhan + "_" + System.currentTimeMillis() + ".docx";
    ...
    private JPanel createAccountPanel() {
        if (!isLoggedIn || UserSession.userId == null || !UserSession.role.equals("bacsi")) {
            JOptionPane.showMessageDialog(null, "Chỉ bác sĩ mới có thể xem thông tin tài khoản!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return new JPanel();
        }
        ...
        try (Connection conn = ConnectionDatabase.getConnection()) {
            String sql = "SELECT bs.hoten, bs.ngaysinh, bs.gioitinh, bs.sdt, tk.email " +
                    "FROM bac_si bs JOIN tai_khoan tk ON bs.sdt = tk.sdt " +
                    "WHERE bs.mabacsi = ?";
        ...
        btnChangePassword.addActionListener(e -> {
            ...
            try (Connection conn = ConnectionDatabase.getConnection()) {
                String sqlVerify = "SELECT mat_khau FROM tai_khoan WHERE sdt = (SELECT sdt FROM bac_si WHERE mabacsi = ?)";
                ...
                String sqlUpdate = "UPDATE tai_khoan SET mat_khau = ? WHERE sdt = (SELECT sdt FROM bac_si WHERE mabacsi = ?)";
