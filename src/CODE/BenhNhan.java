    ...
    // Load data into the fields
        ...
        Runnable loadAppointments = () -> {
            ...
            try (Connection conn = ConnectionDatabase.getConnection()) {
                ...
                // Load patient info
                String query1 = "SELECT * FROM benh_nhan WHERE mabenhnhan = ?";
                ...
                // Load appointments
                String query2 = "SELECT ngay, ten_kham, bac_si, phong, toa, trang_thai FROM lich_hen WHERE ma_benh_nhan = ?";
    ...
    private void loadPatientData(JTextField tfMaBN, JTextField tfHo, JComboBox<String> cbGioiTinh,
        ...
        try (Connection conn = ConnectionDatabase.getConnection()) {
            String sqlBenhNhan = "SELECT mabenhnhan, hoten, gioitinh, ngaysinh, sdt, diachi, cccd, ngayvaovien, ngayravien, nghe_nghiep " +
                    "FROM benh_nhan WHERE mabenhnhan = ?";
            ...
            String sqlPhieuKham = "SELECT ma_bac_si, bac_si_phu_trach, yeu_cau_kham, doi_tuong " +
                    "FROM phieu_kham WHERE ma_benh_nhan = ?";
    ...
    private void loadMedicalRecordData() {
        try (Connection conn = ConnectionDatabase.getConnection()) {
            ...
            String sqlHoso = "SELECT ly_do_vao_vien, tom_tat_benh_ly, tien_su_benh, ket_qua_xet_nghiem, noi_khoa, phau_thuat, tinh_trang_ra_vien " +
                    "FROM hosobenhan WHERE ma_benh_nhan = ?";
    ...
    private JPanel createAccountPanel() {
        ...
        try (Connection conn = ConnectionDatabase.getConnection()) {
            String sql = "SELECT bn.hoten, bn.ngaysinh, bn.gioitinh, bn.sdt, tk.email " +
                    "FROM benh_nhan bn JOIN taikhoan tk ON bn.sdt = tk.sdt " +
                    "WHERE bn.mabenhnhan = ?";
        ...
        btnChangePassword.addActionListener(e -> {
            ...
            try (Connection conn = ConnectionDatabase.getConnection()) {
                // Verify old password
                String sqlVerify = "SELECT mat_khau FROM taikhoan WHERE sdt = (SELECT sdt FROM benh_nhan WHERE mabenhnhan = ?)";
                ...
                // Update password
                String sqlUpdate = "UPDATE taikhoan SET mat_khau = ? WHERE sdt = (SELECT sdt FROM benh_nhan WHERE mabenhnhan = ?)";
